...
thread_local! {
  static MOUSE_EVENT_TX: OnceLock<tokio::sync::mpsc::UnboundedSender<crate::MouseEvent>> = OnceLock::new();
}
...
impl MouseHook {
  ...
  pub fn new(
    window_handle: crate::this::WindowHandle,
  ) -> anyhow::Result<(
    Self,
    Installable<
    ...
    let install = move || {
      ...
      MOUSE_EVENT_TX.with(|tx_lock| {
        tx_lock.set(tx.clone()).map_err(|_| {
  ...
  pub fn handle_mouse_input(
    ...
    let event_tx = MOUSE_EVENT_TX
      .with(|tx_lock| tx_lock.get().cloned())
      .ok_or_else(|| {
